...
int linechecker(para* p, para* q) {
  int k = 0;
  for (int i = p->start, j = q->start; i <= p->stop && j <= q->stop && i != p->filesize && j != q->filesize; ++i, ++j){
  if((k = strcmp(p->base[i], q->base[j])) != 0) { return j; }
  }
  return 1;
}
...
void paraprint(para* p, para* q, int a, void (*fp)(const char*, const char*, int)) {
  if (p == NULL || q == NULL) { return; }
  for (int i = p->start, j = q->start; i <= p->stop && i != p->filesize; ++i, j++) {  if (a == i) { a = 1; } fp(p->base[i], q->base[i], a); }
}
